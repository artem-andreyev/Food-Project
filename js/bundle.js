...
function calc() {
        ...
        initLocalSettings("#gender div", "calculating__choose-item_active"); // div - обращение к блокам внутри селектора
        initLocalSettings(".calculating__choose_big div", "calculating__choose-item_active"); // div - обращение к блокам внутри селектора
        ...
        function getStaticImformation(selector, activeClass) {
            const elements = document.querySelectorAll(selector); // Внутри этого родителя получаем все дивы (parentSelector)
        ...
        getStaticImformation("#gender div", "calculating__choose-item_active"); // div - обращение к блокам внутри селектора
        getStaticImformation(".calculating__choose_big div", "calculating__choose-item_active"); // div - обращение к блокам внутри селектора
...
function cards() {
    ...
    axios__WEBPACK_IMPORTED_MODULE_0__["default"].get("http://localhost:3000/menu") // axios возвращает более подробный ответ
        .then(data => {
...
function slider() {
    ...
    next.addEventListener("click", () => {
        if (offset == deleteNotDigits(width) * (slides.length -1)) { // width = "500px"; slice, reg ex - убрали px
            offset = 0;
        } else {
